...
<div class="card-premium group h-full flex flex-col">
    <div class="relative overflow-hidden aspect-square bg-white p-4 flex-shrink-0">
        <!-- Badges -->
    ...
    <div class="p-4 bg-white flex-grow flex flex-col justify-between border-t border-gray-50">
        <div>
            <div class="mb-1">
            ...
            <h3 class="text-base font-bold text-gray-800 line-clamp-2 group-hover:text-primary transition-colors min-h-[3rem] flex items-start">
                <a href="/products/{{ $product->id }}">{{ $product->title }}</a>
            </h3>
            <div class="min-h-[2.5rem] mt-2 mb-3">
                @if(isset($product->specs) && $product->specs)
                    <p class="text-xs text-gray-500 font-medium leading-tight line-clamp-2">
                        {{ $product->specs }}
                    </p>
                @endif
            </div>
        ...
        <div>
            ...
            <div class="flex items-end justify-between">
                <div>
                    @if($isPhotocopy || !isset($product->sell) || $product->sell <= 0)
                ...
                @if($isPhotocopy || !isset($product->sell) || $product->sell <= 0)
                    <a href="https://zalo.me/0912979394" target="_blank" rel="noopener noreferrer" class="bg-secondary/10 text-secondary p-2 rounded-lg hover:bg-secondary hover:text-white transition-all shadow-sm flex items-center justify-center" title="Liên hệ Zalo">
                        <svg class="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M3 5a2 2 0 012-2h3.28a1 1 0 01.948.684l1.498 4.493a1 1 0 01-.502 1.21l-2.257 1.13a11.042 11.042 0 005.516 5.516l1.13-2.257a1 1 0 011.21-.502l4.493 1.498a1 1 0 01.684.949V19a2 2 0 01-2 2h-1C9.716 21 3 14.284 3 6V5z"></path></svg>
                    </a>
                @else
                    <button onclick="addToCart({{ $product->id }}, this)" class="bg-secondary/10 text-secondary p-2 rounded-lg hover:bg-secondary hover:text-white transition-all shadow-sm flex items-center justify-center cursor-pointer" title="Thêm vào giỏ hàng">
